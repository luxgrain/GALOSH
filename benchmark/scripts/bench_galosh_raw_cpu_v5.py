"""GALOSH RAW CPU v5 robust BayesShrink (MAD-based) bench.

v5 = v1 chromaup (--chroma-up=1) + Pass1 BayesShrink with MAD-based
sigma_Y estimator (--robust-shrink=1).  Aims to suppress the "large grain"
phenomenon where noise that happens to spatially cluster gets compacted
into a few large WHT coefficients, inflates the L2-based sigma_Y, makes
BayesShrink lambda too small, and lets the cluster survive as a "false
signal" blob (the BM3D-CFA-clearable residual).

MAD (median absolute deviation) is robust to outliers — the few large
cluster-induced coefficients sit in the upper tail and don't influence
the median, so sigma_Y stays at the true noise level, sigma_X estimates
0, lambda → infinity, and the cluster is killed.  Donoho-Johnstone (1995)
robust noise estimation transplanted from wavelet shrinkage to GALOSH's
local WHT-LOSH.  Local-only — preserves GPU/streaming compatibility.

Output:
    benchmark/sidd_medium/galosh_raw_cpu_v5_robust_mad/<tag>_den.png

Per-image pipeline identical to v1; only --robust-shrink=1 differs.
"""
...
TMP_DIR     = GALOSH_ROOT / "benchmark" / "sidd_medium" / "_tmp_v5bench"
TMP_DIR.mkdir(parents=True, exist_ok=True)

UCRT_BIN    = r"C:\msys64\ucrt64\bin"

VARIANTS = [
    # method_dir,                       chroma_up, robust_shrink
    ("galosh_raw_cpu_v5_robust_mad",    1,         1),
]
...
def run_galosh_raw_cpu(noisy_raw, w, h, uid, chroma_up, robust_shrink,
                        omp_threads):
...
def process_one(args):
    ...
    rows = []
    for method_dir, chroma_up, robust_shrink in VARIANTS:
...
def main():
    ...
    print(f"=== GALOSH RAW CPU v5 robust-MAD bench ===")
    print(f"  exe       : {EXE}")
    ...
    metrics_path = OUT_ROOT / "_metrics_v5bench.json"
