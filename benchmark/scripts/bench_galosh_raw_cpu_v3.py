"""GALOSH RAW CPU v3 chromawhtlosh bench.

v3 = v1 chromaup (--chroma-up=1) + WHT-LOSH chroma denoise with low-freq
2x2 protection (--chroma-method=wht_losh_cfa), replacing LOESS.  Aims to
fix saturation drop on heavy-noise flat regions by switching from LOESS's
mean estimator (which dilutes saturation when the bilateral window
straddles color patch boundaries) to block-coefficient shrinkage that
preserves the lowest-frequency 2x2 sub-band of each 8x8 chroma block.

Output:
    benchmark/sidd_medium/galosh_raw_cpu_v3_chromawhtlosh/<tag>_den.png

Per-image pipeline identical to v1/v2 (Menon demosaic + per-scene 3x4
affine + sRGB gamma) -- only --chroma-method changes.
"""
...
TMP_DIR     = GALOSH_ROOT / "benchmark" / "sidd_medium" / "_tmp_v3bench"
TMP_DIR.mkdir(parents=True, exist_ok=True)

UCRT_BIN    = r"C:\msys64\ucrt64\bin"

VARIANTS = [
    # method_dir,                              chroma_up, chroma_method
    ("galosh_raw_cpu_v3_chromawhtlosh",        1,         "wht_losh_cfa"),
]
...
def run_galosh_raw_cpu(noisy_raw, w, h, uid, chroma_up, chroma_method,
                        omp_threads):
...
def process_one(args):
    ...
    rows = []
    for method_dir, chroma_up, chroma_method in VARIANTS:
        ...
        print(f"  [{tag} {method_dir:<37s}] {dt_ms*1000:7.1f}ms PSNR_gt2={psnr_gt2:.3f} PSNR_isp={psnr_isp:.3f}",
              flush=True)
    return rows
...
def main():
    ...
    print(f"=== GALOSH RAW CPU v3 chromawhtlosh bench ===")
    print(f"  exe       : {EXE}")
    ...
    for method, rs in by_method.items():
        ...
        print(f"  {method:<39s} N={len(rs):2d}  mean_ms={ms:7.1f}  PSNR_gt2={gt2:.3f}  PSNR_isp={isp:.3f}")

    metrics_path = OUT_ROOT / "_metrics_v3bench.json"
