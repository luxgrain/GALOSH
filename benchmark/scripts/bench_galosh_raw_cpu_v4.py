"""GALOSH RAW CPU v4 K13-4x4 grain-scale-matched bench.

v4 = v1 chromaup (--chroma-up=1) + K13 (half-res L Pass1+Pass2) using
4x4 WHT-LOSH instead of 8x8 (--k13-block=4).  Goal: align K13's full-res
grain scale (= 2 × 4 = 8 px) with K15's 8x8 native scale (= 8 px),
restoring grain-scale unity that the K11 fixed-2x2 Bayer hierarchy
implies but legacy 8x8/8x8 violated (yielding 16-px K13 grain that
K15's 8-px Wiener treats as smooth signal and preserves).

When --k13-block=4, halfres_stride defaults to 1 (instead of 2) so that
overlap-add quality matches the legacy 8x8/stride=2 (75% overlap).

Output:
    benchmark/sidd_medium/galosh_raw_cpu_v4_k13_4x4/<tag>_den.png

Per-image pipeline identical to v1/v2/v3; only --k13-block=4 changes.
"""
...
TMP_DIR     = GALOSH_ROOT / "benchmark" / "sidd_medium" / "_tmp_v4bench"
TMP_DIR.mkdir(parents=True, exist_ok=True)

UCRT_BIN    = r"C:\msys64\ucrt64\bin"

VARIANTS = [
    # method_dir,                           chroma_up, k13_block
    ("galosh_raw_cpu_v4_k13_4x4",           1,         4),
]
...
def process_one(args):
    ...
    rows = []
    for method_dir, chroma_up, k13_block in VARIANTS:
...
def main():
    ...
    print(f"=== GALOSH RAW CPU v4 K13-4x4 bench ===")
    print(f"  exe       : {EXE}")
    ...
    metrics_path = OUT_ROOT / "_metrics_v4bench.json"
