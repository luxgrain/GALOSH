"""Per-variable Q design — Phase B: chroma line-buffer precision requirement.

Builds the HP probe with the chroma line buffers (L_cs, pilot, L_den, L_pixel,
L_h_den) rounded to N fractional bits (= INT16 Q(int).N data path), compute
stays int64.  Sweep N; the N where PSNR converges to HP-full = the chroma frac
bits the data path actually needs.  INT16 chroma allows ~6 frac (9 int bits).
"""
...
GALOSH = Path(r"C:\Users\luxgrain\GALOSH"); STD = GALOSH / "standalone"
...
print("--- chroma line buffer rounded to N frac bits (INT16 chroma ~ N=6) ---")
for frac in FRACS:
    e = build([f"-DCHROMA_FRAC={frac}"], f"chroma{frac}")
    if not e: continue
    row = []
    for t in PATCHES:
        n, g, h, w, pf = data[t]
        row.append(f"{psnr(g, run(e, n, w, h, 'r32')):>10.2f}")
    print(f"{'chroma frac='+str(frac):>16} " + " ".join(row))
    e.unlink(missing_ok=True)
exe_full.unlink(missing_ok=True)
print("\n(find smallest chroma frac that matches HP full → the chroma data-path frac requirement)")
