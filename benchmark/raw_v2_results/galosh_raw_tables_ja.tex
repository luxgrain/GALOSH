...
% FP32 = o32 GPU、INT16 = i16 GPU（lf=5 cf=9、r32 CPU 参照に対し zero-loss）。
...
--- クロスプラットフォームの基準点である。$^{\dagger}$INT16 の品質は bit-exact
な固定小数点パイプラインの値。CPU 時間は正確性優先の INT32 参照実装
（速度未最適化）、GPU 時間はストリーミング i16 実装（\ref{sec:map}節参照）。}
\end{table}
...
% パイプライン、1493/1493、err 0（E:\rawnind_bench_v2\_metrics_revst_gpu.json）。
...
参照実装は CUDA 必須で CPU では動作しない。bit-exact な INT16 固定小数点版
