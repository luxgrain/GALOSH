...
\section*{謝辞}
本稿のテキストは、著者の指示・監督のもと Anthropic 社の大規模言語モデル
Claude（Claude Opus 4.7/4.8 および Claude Fable 5）の支援を受けて起草され、
参照実装も同じ AI 支援のもとで開発された（公開リポジトリのコミット履歴に
逐次記録）。手法・実験・すべての主張の設計・検証・承認は著者が行い、内容の
全責任は著者が負う。
