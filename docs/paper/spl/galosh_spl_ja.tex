...
\texttt{https://github.com/luxgrain/GALOSH}。本レターは著者の拡張プレプリント
arXiv:2607.03768~\cite{galosharxiv}（全データセットの完全な表・ISO 帯別内訳・
速度検証を含む）の凝縮版である。}}
...
動作するソフトウェア INT16 固定小数点実装がこれを裏付ける。
\end{abstract}
...
壊滅的に --- 劣化する（本稿でも実測される）。近年のブラインド raw
パイプラインはデータ要件を緩めるが、中核には学習済みネットワークを残して
いる~\cite{yond,noise2vst}。BM3D/NLM 以外にも古典系には
...
演算子を使い、独立した強度制御を公開する。同じコアが 2 つの薄い前段に仕える。raw 側の
\textbf{GALOSH-RAW} は通常の処理順序を逆転させる: CFA を「RGB の欠損データ」
--- デノイズの前にデモザイクで解くべき補間問題 --- として扱う代わりに、
Bayer モザイクを空間多重化された輝度/色差信号として扱い（周波数領域 CFA 解析
~\cite{alleysson,dubois,hirakawa}の見方の系譜）、サイクルスピン $2{\times}2$ Walsh--Hadamard
変換で\emph{厳密に}分離する --- 輝度はフル解像度、色差は半解像度 ---
これにより 2 種類のノイズ集団を、いかなる
デモザイカが走るより前に別々に扱える。\textbf{GALOSH-YUV/RGB} は同じ分離を
線形光 BT.709 変換でレンダリング済み sRGB 画像に適用する。本稿の主張は
...
\subsection{ドメイン前段}
\paragraph{GALOSH-RAW} Bayer モザイクを「未完成の RGB」ではなく輝度/色差
多重化信号として直接扱う: ブラインドフィットと GAT の
後、CFA 対応の無彩色ダークリファレンス（無彩色フロアの IRLS 推定）を減算し、
stride-1 サイクルスピン $2{\times}2$ Walsh--Hadamard 変換がモザイクを
1 つの\emph{フル解像度}輝度プレーン --- 各画素位置で評価した無彩色
$2{\times}2$ 平均。クワッド整列変換から stride-1 スライディング DC として
厳密に導出される --- と、整列クワッド上の 3 つの半解像度色差プレーンへ
分離する。この分解は直交かつ厳密可逆であり、補間は一切介在しない。モザイク
そのものへの成分変換デノイジングには先行例がある~\cite{hirakawa,park09}が、
本稿の新規性は、厳密なサイクルスピン分離を完全ブラインド・探索不要の
2 演算子コアへ接続した点にある。色差は半解像度
...
raw 出力は全手法同一の画像ごと較正レンダラーで sRGB へレンダリングして計測する。
...
sRGB 領域ブラインド・デノイジング。太字 / \underline{下線} = blind・
training-free 手法中の最良 / 2 位。学習済みネットワークは上限参照（NAFNet と
...
ノイジーなサブセット（ISO${\geq}6400$、$n{=}30$）で、CBM3D と Color-NLM にグラウンド
...
NLM-CFA/BM3D-CFA の $3.21$/$6.33$\,s --- 計測した手法の中で両プラットフォームとも
最速である（計測プロトコルと完全な速度表は~\cite{galosharxiv}）。
...
\section{結論}
古典的デノイジングから探索を取り除き --- ブロックマッチングを分散安定化された
サイクルスピン局所シュリンケージと輝度ガイド色差回帰で置き換え、CFA デノイズを
「欠損 RGB の補間」から「厳密な輝度/色差分離」へ定義し直す --- ことで、
...
\section*{謝辞}
本稿のテキストと参照実装は、著者の指示・監督のもと Anthropic 社の Claude
モデル（Opus 4.7/4.8、Fable 5）の支援を受けて作成された（リポジトリの
コミット履歴に逐次記録）。手法・実験・全主張の設計・検証・承認は著者が行い、
内容の全責任は著者が負う。
...
\caption{sRGB ドメイン、SIDD Medium（フルフレーム）。列: Noisy / CBM3D /
Color-NLM / Guided / NAFNet-SIDD / SCUNet / Restormer-SIDD /
\textbf{GALOSH-YUV（提案）} / GT。画像ごとの LPIPS（$\downarrow$）付き。全手法
ブラインド。古典的ベースラインは相関したレンダリングノイズに大きな残留ノイズを
残し、GALOSH はそれらと SIDD 学習ネットワークの間に位置する。}
\label{fig:qualsrgb}
\end{figure*}
