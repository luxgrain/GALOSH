...
\texttt{https://github.com/luxgrain/GALOSH}. This letter is a condensed
presentation of the author's extended preprint arXiv:2607.03768~\cite{galosharxiv},
which contains the full per-dataset tables, per-ISO breakdowns, and the
speed study.}}
...
software INT16 fixed-point realization.
\end{abstract}

\begin{IEEEkeywords}
Image denoising, blind denoising, variance stabilization, raw image
processing, fixed-point arithmetic.
\end{IEEEkeywords}
...
training distribution, which we also observe; recent blind raw pipelines
relax the data requirement but keep a trained network at the
core~\cite{yond,noise2vst}. Beyond BM3D/NLM, the classical
...
strength controls. The same core serves two thin front-ends. On raw,
\textbf{GALOSH-RAW} inverts the usual order of operations: instead of
treating the CFA as RGB with missing samples --- an interpolation problem
--- it treats the Bayer mosaic as a spatially multiplexed luma/chroma
signal, in the spirit of frequency-domain CFA analysis~\cite{alleysson,dubois,hirakawa},
and demultiplexes it \emph{exactly} with a cycle-spun $2{\times}2$
Walsh--Hadamard transform --- so the two noise populations are treated
differently before any demosaicer runs. \textbf{GALOSH-YUV/RGB} applies
the same split to rendered sRGB images via a linear-light BT.709
transform. Our claim is not any single component but their
...
mean over overlapping blocks (a Foi-style lower-envelope
estimator~\cite{foi}; per-CFA-channel blocks in the raw pipeline, a
single-plane variant on luminance in the YCbCr pipeline), yielding
...
\paragraph{GALOSH-RAW} operates directly on the Bayer mosaic, treated as a
luma/chroma-multiplexed signal rather than incomplete RGB: after the blind
...
Walsh--Hadamard transform demultiplexes the mosaic into one
\emph{full-resolution} luminance plane --- the achromatic $2{\times}2$
average at every pixel, an exact stride-1 sliding DC of the quad-aligned
transform --- and three half-resolution chroma planes on the aligned
quads: orthogonal, exactly invertible, no interpolation. Component-transform
denoising of the mosaic itself has precedents~\cite{hirakawa,park09}; what is
new here is the exact cycle-spun demultiplex feeding a fully blind,
search-free two-operator core. Chroma is denoised at
...
(LPIPS~\cite{lpips}) on sRGB --- raw outputs are rendered to sRGB with the
same per-image calibrated renderer for every method; on full-frame sRGB the learned baselines run
...
${\sim}15.8$\,MP frames). Bold / \underline{underline} = best / 2nd-best among
the blind, training-free methods; the trained networks are an upper reference
(NAFNet and Restormer are trained \emph{on} SIDD).}
\label{tab:srgb}
...
model. We also re-ran the noisiest RawNIND subset (ISO${\geq}6400$, $n{=}30$) giving
...
1080p it takes $1.38$\,s vs.\ $3.21$/$6.33$\,s for NLM-CFA/BM3D-CFA --- among
the measured methods, the fastest on both platforms
(measurement protocol and full timing tables in~\cite{galosharxiv}).
...
chroma regression, and recasting CFA denoising from interpolation over
missing RGB into an exact luma/chroma demultiplex --- retains the
classical family's quality (surpassing the
...
The manuscript text and the reference implementation were prepared with the
assistance of Anthropic's Claude models (Opus 4.7/4.8, Fable 5) under the
author's direction, documented per-commit in the repository. The method,
experiments, and all claims were designed, verified, and approved by the
...
\clearpage
\begin{thebibliography}{26}\small
...
\emph{CVPR}, 2018.
\bibitem{alleysson} D.~Alleysson, S.~S\"usstrunk, and J.~H\'erault,
``Linear demosaicing inspired by the human visual system,'' \emph{IEEE
TIP}, 2005.
\bibitem{galosharxiv} Y.~Sato, ``GALOSH: Blind, training-free denoising of raw
Bayer and sRGB images by parallel-friendly local shrinkage,'' arXiv:2607.03768
[eess.IV], 2026.
\bibitem{yond} H.~Feng, L.~Wang, Y.~Huang, T.~Li, L.~Zhu, and H.~Huang,
``YOND: Practical blind raw image denoising free from camera-specific data
dependency,'' arXiv:2506.03645 [eess.IV], 2025.
\bibitem{noise2vst} S.~Herbreteau and M.~Unser, ``Self-calibrated
variance-stabilizing transformations for real-world image denoising,''
\emph{ICCV}, 2025.
\bibitem{dubois} E.~Dubois, ``Frequency-domain methods for demosaicking of
Bayer-sampled color images,'' \emph{IEEE SPL}, 2005.
\bibitem{hirakawa} K.~Hirakawa, X.-L.~Meng, and P.~J.~Wolfe, ``A framework for
wavelet-based analysis and processing of color filter array images with
applications to denoising and demosaicking,'' \emph{ICASSP}, 2007.
\bibitem{park09} S.~H.~Park, H.~S.~Kim, S.~Lansel, M.~Parmar, and
B.~A.~Wandell, ``A case for denoising before demosaicking color filter array
data,'' \emph{Asilomar Conf.\ Signals, Syst., Computers}, 2009.
\end{thebibliography}
