...
to be demosaiced before denoising --- it treats the Bayer mosaic as a
spatially multiplexed luma/chroma signal, in the spirit of frequency-domain
CFA analysis~\cite{alleysson}, and demultiplexes it \emph{exactly} with a cycle-spun
$2{\times}2$ Walsh--Hadamard transform --- full-resolution luminance,
half-resolution chroma --- so the two noise
populations can be treated differently before any demosaicer runs.
\textbf{GALOSH-YUV/RGB} applies the same split to rendered sRGB images via
a linear-light BT.709 transform. Our claim is not any single component but their
...
average evaluated at every pixel position, obtained exactly from the
quad-aligned transform as a stride-1 sliding DC --- and three
half-resolution chroma planes on the aligned quads; the decomposition is
orthogonal and exactly invertible, with no interpolation involved. Chroma is denoised at
