% ============================================================================
% GALOSH — arXiv 原稿 日本語版（確認・査読用。投稿は英語版）
...
% (pandoc の HTML 化はこのブロックを無視するので安全)。太字は和文慣習で
% ゴシック代用。Yu Mincho/Yu Gothic = Windows 標準搭載。
\usepackage{xeCJK}
...
比較した blind・training-free 手法の中で一貫して最良であり、オラクルのノイズ
レベルを与えた BM3D/NLM 系すら上回る。raw では学習済みネットワークに肉薄し、sRGB の
...
\item 実ノイズ 4 ベンチマークにわたり、GALOSH が両ドメインで比較した blind・
training-free 手法中最良であるという証拠 --- オラクル $\sigma$ を与えた古典的
...
他方を操舵する。
BM3D/NLM 以外にも古典系には低ランクパッチモデル（WNNM~\cite{wnnm}）、
パッチ事前分布最適化（EPLL~\cite{epll}）、学習辞書（K-SVD~\cite{ksvd}）、
ブラインドの Noise Clinic パイプライン~\cite{noiseclinic} があるが、いずれも
同様に探索ないし最適化が重く、本稿の比較では最も広く再現されている代表として
BM3D/NLM（CFA 対応・カラー変種を含む）を採用する。
...
フルフレーム sRGB（$\sim$15.8\,MP）では DL ベースラインはタイル推論
（オーバーラップ＋フェザリング）で走らせ、LPIPS/DISTS は\emph{全手法}で同一の
$1024^2$ タイル格子上の平均として報告する --- この解像度での GPU メモリと指標
実装の制約による統一プロトコルであり、手法ごとの選択ではない。
...
数値発散する（クリップ前のネットワーク生出力が $\pm10^3$ に達する）。GALOSH を含む他の全ベースラインは同じ入力で
...
モデルによれば、ピクセルあたり演算数は $0.5$--$1$\,TMAC/s の固定機能演算予算
--- 現在の ISP 水準では控えめ --- の下で 1080p--4K 級ストリーミングと両立し、
オンチップ状態は最大垂直カーネルのライン
...
探索不要並列・高速 --- を、オラクル条件下でも比較した古典系を上回る品質で同時に満たす
...
\input{galosh_srgb_tables_ja.tex}
