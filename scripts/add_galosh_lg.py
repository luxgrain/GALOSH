#!/usr/bin/env python3
"""
Benchmark GALOSH with luma-guided chroma Wiener.
Runs specified (luma_str, chroma_str) configuration.

Saves images as *_GALOSH_LG{L}{C}.png (e.g. GALOSH_LG05_10, GALOSH_LG10_10)
Saves metrics to results/v6lg{tag}_{dataset}_results.json
"""
import numpy as np
import subprocess
import os
...
def run_galosh(noisy_raw, w, h, alpha, sigma_sq, luma_str, chroma_str, uid="0"):
    inp = str(BASE / "standalone" / f"tmp_galosh_lg_{uid}.bin")
    out = str(BASE / "standalone" / f"tmp_galosh_lg_out_{uid}.bin")
    noisy_raw.tofile(inp)
    cmd = [EXE_V6, inp, out, str(w), str(h), "galosh",
           "1.0", str(luma_str), str(chroma_str),
           str(alpha), str(sigma_sq), "1", "25"]
...
def main():
    parser = argparse.ArgumentParser(description="GALOSH luma-guided chroma benchmark")
    parser.add_argument("--dataset", "-d", default="kodak",
                        choices=list(DATASETS.keys()))
    parser.add_argument("--luma-str", "-l", type=float, default=0.5)
    parser.add_argument("--chroma-str", "-c", type=float, default=1.0)
    args = parser.parse_args()

    ds_name = args.dataset
    ds = DATASETS[ds_name]
    IMGDIR = ds["path"]
    ls, cs = args.luma_str, args.chroma_str

    # Tag for filenames: e.g. "05_10" for L0.5/C1.0
    tag = f"{ls:.1f}_{cs:.1f}".replace(".", "")
    ...
    method_label = f"GALOSH_LG{tag}"
    print(f"{method_label} benchmark [{ds_name}] ({n_images} images x {len(isos)} ISOs)")
    print(f"  sigma_L={ls}, sigma_C={cs}")
    print(f"  exe: {EXE_V6}")
    ...
    for iso in isos:
        ...
        for idx, img_path in enumerate(images):
            ...
            uid = f"{ds_name}_L{ls}_iso{iso}_img{idx}"
            ...
            imsave(str(iso_dir / f"{img_path.stem}_{method_label}.png"),
        ...
        all_results.append({"iso": iso, **{f"{method_label}_{k}": v for k, v in avg.items()}})

    # Save
    RESULTS.mkdir(exist_ok=True)
    out_path = RESULTS / f"v6lg{tag}_{ds_name}_results.json"
