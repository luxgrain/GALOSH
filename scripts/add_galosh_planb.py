#!/usr/bin/env python3
"""
Add GALOSH Plan B results to existing comparison_images and JSON.
Plan B: full-res raw GALOSH (luma) + half-res chroma GALOSH.
Saves images as *_GALOSH_B.png into comparison_images_v5_{dataset}.
"""
import numpy as np
import subprocess
import os
...
def run_galosh(noisy_raw, w, h, alpha, sigma_sq, uid="0"):
    inp = str(BASE / "standalone" / f"tmp_galoshb_{uid}.bin")
    out = str(BASE / "standalone" / f"tmp_galoshb_out_{uid}.bin")
...
def main():
    parser = argparse.ArgumentParser(description="Add GALOSH Plan B results")
    ...
    # Save images to v5 comparison folder
    ...
    print(f"GALOSH Plan B benchmark [{ds_name}] ({n_images} images x {len(isos)} ISOs)")
    ...
    for iso in isos:
        ...
        for idx, img_path in enumerate(images):
            ...
            imsave(str(iso_dir / f"{img_path.stem}_GALOSH_B.png"),
        ...
        all_results.append({"iso": iso, **{f"GALOSH_B_{k}": v for k, v in avg.items()}})

    # Save
    out_path = RESULTS / f"v6b_{ds_name}_results.json"
