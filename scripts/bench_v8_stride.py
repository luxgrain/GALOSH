#!/usr/bin/env python3
"""
Benchmark for GALOSH_F stride=2 ablation study.

Three variants:
  F_HALF: stride=2 on half-res L/C only (full-res L uses stride=4)
  F_FULL: stride=2 on full-res L only (half-res L/C uses stride=4)
  F:      stride=2 on both half-res L/C and full-res L

Compares against v7b baseline (stride=4 everywhere, GALOSH_FULLRES_L).

Usage:
  python bench_v8_stride.py -d kodak -v F_HALF -p 10_10
  python bench_v8_stride.py -d kodak -v all -p 10_10
"""
import numpy as np
import subprocess
import os
...
VARIANTS = {
    "F_HALF": BASE / "standalone" / "rawdenoise_v8_fhalf.exe",
    "F_FULL": BASE / "standalone" / "rawdenoise_v8_ffull.exe",
    "F":      BASE / "standalone" / "rawdenoise_v8_f.exe",
}
...
def run_denoise(exe, noisy_raw, w, h, alpha, sigma_sq, ls, cs, uid="0"):
    inp = str(BASE / "standalone" / f"tmp_v8_{uid}.bin")
    out = str(BASE / "standalone" / f"tmp_v8_out_{uid}.bin")
...
def run_benchmark(ds_name, variant, ls, cs):
    ...
    for iso in isos:
        ...
        all_results.append({
            ...
            **{f"{method_name}_{k}": v for k, v in avg.items()}
        })

    # Save
    out_path = RESULTS / f"v8_{variant.lower()}_{ls_tag}_{cs_tag}_{ds_name}_results.json"
