#!/usr/bin/env python3
"""
Benchmark for GALOSH color-shift fixes: 案A (CFA protect) and 案B (fullres L).

Runs both rawdenoise_v7a.exe (案A) and rawdenoise_v7b.exe (案B) with two
parameter sets: sigma_L=0.5/sigma_C=1.0 and sigma_L=1.0/sigma_C=1.0.

Saves images as *_GALOSH_A_{ls}_{cs}.png and *_GALOSH_B_{ls}_{cs}.png.
Saves metrics to results/v7{a,b}_{ls}_{cs}_{dataset}_results.json.
"""
import numpy as np
import subprocess
import os
...
EXE_A = str(BASE / "standalone" / "rawdenoise_v7a.exe")
EXE_B = str(BASE / "standalone" / "rawdenoise_v7b.exe")
...
def run_denoise(exe, noisy_raw, w, h, alpha, sigma_sq, ls, cs, uid="0"):
    inp = str(BASE / "standalone" / f"tmp_v7ab_{uid}.bin")
    out = str(BASE / "standalone" / f"tmp_v7ab_out_{uid}.bin")
...
def run_benchmark(ds_name, variant, ls, cs):
    """Run benchmark for one variant + one parameter set."""
    exe = EXE_A if variant == "A" else EXE_B
    ds = DATASETS[ds_name]
    IMGDIR = ds["path"]
    OUTDIR = BASE / f"comparison_images_v7_{ds_name}"
    ...
    print(f"\n{'='*60}")
    print(f"{method_name} benchmark [{ds_name}]")
    ...
    # Save
    out_path = RESULTS / f"v7{variant.lower()}_{ls_tag}_{cs_tag}_{ds_name}_results.json"
...
def main():
    parser = argparse.ArgumentParser(description="GALOSH v7 A/B color-shift fix benchmark")
    parser.add_argument("--dataset", "-d", default="kodak",
                        choices=list(DATASETS.keys()))
    parser.add_argument("--variant", "-v", default="both",
                        choices=["A", "B", "both"],
                        help="Which variant: A (CFA protect), B (fullres L), or both")
    parser.add_argument("--params", "-p", default="both",
                        choices=["05_10", "10_10", "both"],
                        help="Parameter set: 05_10 (ls=0.5,cs=1.0), 10_10 (ls=1.0,cs=1.0), or both")
    ...
    param_sets = [(0.5, 1.0), (1.0, 1.0)] if args.params == "both" else \
                 [(0.5, 1.0)] if args.params == "05_10" else [(1.0, 1.0)]

    for variant in variants:
        for ls, cs in param_sets:
            run_benchmark(args.dataset, variant, ls, cs)
