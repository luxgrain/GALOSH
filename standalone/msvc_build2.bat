...
"%CL_PATH%" /nologo /c /O2 /openmp /TC /I"%MSVC_INC%" /I"%SDK_UCRT_INC%" /I"%SDK_UM_INC%" /I"%SDK_SH_INC%" yuv_galosh_core.c /Fo:yuv_galosh_core_msvc.obj
if errorlevel 1 ( echo C_COMPILE_FAILED & exit /b 1 )
echo C_OK

"%CL_PATH%" /nologo /LD /O2 /openmp /EHsc /I"%MSVC_INC%" /I"%SDK_UCRT_INC%" /I"%SDK_UM_INC%" /I"%SDK_SH_INC%" /I"%WORK%" galosh_avisynth.cpp yuv_galosh_core_msvc.obj /Fe:GALOSHDenoise_msvc.dll /link /LIBPATH:"%MSVC_LIB%" /LIBPATH:"%SDK_UCRT_LIB%" /LIBPATH:"%SDK_UM_LIB%"
