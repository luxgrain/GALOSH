...
static void fxp_gat_build_inverse_table(const fxp_gat_params *gat_p) {
  ...
  for(int i = 0; i < FXP_GAT_INV_TABLE_SIZE; i++) {
    ...
    for(int k = 0; k <= k_max; k++) {
       ...
       * to E[T]).  Saves 20% of inner-loop work with no measurable quality
       * impact (verified: v12 with full 10-point gave 49.10 SIDD; 8-point
       * gives same to 3 decimals). */
      fxp_acc eg_acc = fxp_acc_zero();
      fxp32 k_alpha = (fxp32)k * gat_p->alpha;
      for(int g = 1; g < 9; g++) {
        fxp32 z = fxp_mul(sqrt2_sigma, fxp_gh_nodes_q20[g]);
        fxp32 noisy_y = k_alpha + z;
