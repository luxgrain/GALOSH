...
/* Precompute composite constants from α (Q11.20 unscaled) and σ²_scaled
 * (Q11.20 of 1024 × σ²_real).  Internal scaling absorbs the 1024× in the
 * σ²/α and 2σ/α computations, preserving precision for σ² < 1 LSB.
 *
 * Bug 11 fix (2026-05-13): σ² for super-clean signals (RawNIND ISO 100-800,
 * σ² ≈ 1e-7) underflows Q11.20 (LSB = 9.5e-7), causing >>10 unscale at
 * Phase 0 output to round to 0.  Then defensive floor σ²→1 raw made σ²
 * 56× too big, inflating σ²/α and corrupting Phase 1+.
 *
 * Fix: keep σ²_scaled (= 1024 × σ²_real) all the way through Phase 0 output,
 * and have fxp_gat_precompute_scaled handle the 1024× factor internally:
 *   σ²/α = σ²_scaled / (1024 × α)  (= fxp_div_q20 with α × 1024 denominator)
 *   σ_real = sqrt(σ²_scaled) / 32    (= sigma_raw_scaled >> 5 in Q11.20 raw)
 *   t_break = σ_scaled / (16 × α)   (= fxp_div_q20)
 *
 * 超清浄信号 σ²<1e-7 が Q11.20 で underflow し σ²/α 過大化していたバグの修正。
 * Phase 0 内部の var_scale 1024× をそのまま GAT params 計算まで持ち越し、
 * fxp_div_q20 で適切な分母 (α×1024, α×16) を使うことで完全な精度を保つ。 */
static inline void fxp_gat_precompute_scaled(fxp_gat_params *p,
                                              fxp32 alpha, fxp32 sigma_sq_scaled) {
  p->alpha = alpha;
  /* σ² stored as best-effort unscaled Q11.20 (for display only; downstream
   * uses σ²/α and σ_raw which are computed precisely from scaled input). */
  p->sigma_sq = sigma_sq_scaled >> 10;
  if(sigma_sq_scaled <= 0) sigma_sq_scaled = 1;
  if(alpha <= 0)    alpha    = 1;
  /* σ_scaled = sqrt(σ²_scaled) = √1024 × σ_real = 32 × σ_real (in Q11.20 raw). */
  fxp32 sigma_scaled = fxp_sqrt(sigma_sq_scaled);
  /* σ_raw (real) = σ_scaled / 32, in Q11.20 raw via >>5. */
  p->sigma_raw = sigma_scaled >> 5;
  ...
  /* σ²/α (real) = σ²_scaled / (1024 × α).  α × 1024 fits int32 for α < 2.
   * Result has full precision even when σ²_real is below Q11.20 LSB. */
  fxp32 alpha_x_1024 = alpha * 1024;
  if(alpha_x_1024 <= 0) alpha_x_1024 = 1024;
  p->sigma_sq_over_alpha = fxp_div_q20(sigma_sq_scaled, alpha_x_1024);
  p->y_break = -p->three_alpha_8;
  /* t_break = 2σ_real/α_real = σ_scaled / (16 × α_real). */
  fxp32 alpha_x_16 = alpha * 16;
  if(alpha_x_16 <= 0) alpha_x_16 = 16;
  p->t_break = fxp_div_q20(sigma_scaled, alpha_x_16);
  /* inv_sigma_raw = 1/σ_real.  For very small σ_real, may overflow Q11.20.
   * Used only in linear-branch GAT inverse (D < t_break).  For super-clean
   * scenes where σ_real is small, t_break is also small, so linear branch
   * is rarely hit.  Accept saturation here. */
  p->inv_sigma_raw = (p->sigma_raw > 0) ? fxp_recip(p->sigma_raw) : FXP_MAX_INT;
}

/* Legacy wrapper: caller passes σ²_real_q20 (= unscaled Q11.20), we scale
 * up by 1024 internally before delegating to fxp_gat_precompute_scaled. */
static inline void fxp_gat_precompute(fxp_gat_params *p,
                                      fxp32 alpha, fxp32 sigma_sq) {
  fxp_gat_precompute_scaled(p, alpha, sigma_sq * 1024);
}

/* DEPRECATED original — kept under #if 0 for diff reference. */
#if 0
static inline void fxp_gat_precompute_OLD(fxp_gat_params *p,
                                      fxp32 alpha, fxp32 sigma_sq) {
  p->alpha = alpha;
  p->sigma_sq = sigma_sq;
...
#endif  /* end #if 0 deprecated block */
...
static inline fxp32 fxp_gat_forward(fxp32 x_q20,
  ...
  if(x_q20 >= p->y_break) {
     ...
     *
     * Bug 12 fix (2026-05-14): use fxp_div_q20 instead of fxp_mul(dx,
     * inv_sigma_raw).  For very small σ_real (< ~5e-4 typical for super-
     * clean RawNIND), inv_sigma_raw = 1/σ > 2048 = Q11.20 max → saturates
     * to FXP_MAX_INT.  Multiplying by saturated value gives wildly wrong T,
     * corrupting Foi LUT build (= linear branch hit at z < y_break for
     * tiny σ).  Direct division has no Q11.20 range issue. */
    fxp32 dx = x_q20 - p->y_break;
    fxp32 sigma_safe = (p->sigma_raw > 0) ? p->sigma_raw : 1;
    return p->t_break + fxp_div_q20(dx, sigma_safe);
  }
}
