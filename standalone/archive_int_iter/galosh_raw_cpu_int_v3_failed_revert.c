...
static void fxp_gat_build_inverse_table(const fxp_gat_params *gat_p) {
  /* Skip LUT build if α < 1e-4 (= Phase 0 likely floored / failed to estimate).
   * For very small α, σ²/α scaling causes LUT build to produce nonsensical
   * d ranges → catastrophic Phase 10 output.  Fall back to algebraic. */
  ...
  /* Precompute z_table[g] = sqrt(2) × σ × node[g] for ALL 10 GH points
   * (v12 baseline used full 10-point integration).  Tried 8-point in v13
   * cleanup but contributed to RawNIND regression. */
  fxp32 z_table[10];
  for(int g = 0; g < 10; g++) {
    z_table[g] = fxp_mul(sqrt2_sigma, fxp_gh_nodes_q20[g]);
  }
  ...
  for(int i = 0; i < FXP_GAT_INV_TABLE_SIZE; i++) {
    ...
    /* k_max ≈ λ + 8·sqrt(λ) + 20 (v12 baseline).  Tried 6σ in v13 cleanup
     * but contributed to RawNIND regression. */
    int lambda_int = lambda >> FXP_FRAC;
    fxp32 sqrt_lambda = (lambda > 0) ? fxp_sqrt(lambda) : 0;
    int sqrt_lambda_int = sqrt_lambda >> FXP_FRAC;
    int k_max = lambda_int + 8 * sqrt_lambda_int + 20;
    ...
    for(int k = 0; k <= k_max; k++) {
      ...
      /* Inner: 10-point Gauss-Hermite over Gaussian noise of stddev σ.
       * Uses precomputed z_table to avoid per-k fxp_mul of sqrt(2)·σ·node
       * (= k-independent).  v12 baseline used full 10-point. */
      ...
      for(int g = 0; g < 10; g++) {
        ...
        fxp_acc_madd(&eg_acc, fxp_gh_weights_q20[g], T);
      }
...
static void phase0c_wls_solve(const fxp32 *bin_mean_arr,
  ...
  /* Bug 13 reverted (2026-05-14): matched v12 baseline.  Re-investigate with
   * isolated Bug 11 fix in future r16 session. */
  ...
  /* Bug 11 reverted (2026-05-14): output σ² UNSCALED to match v12 behavior.
   * RawNIND regression observed at applebag-like patches (25 dB vs FP32 31 dB)
   * with Bug 11 enabled; v12 baseline 29.37 dB aggregate held with unscaled
   * output.  Bug 11 was meant to recover super-clean RawNIND (s09-like) but
   * net negative on aggregate.  Re-investigate in future r16 session where
   * σ² precision is naturally tighter. */
  alpha_est    = alpha_est    >> 10;     /* /1024 */
  sigma_sq_est = sigma_sq_est >> 10;     /* /1024 — restored Bug 11 revert */
...
static void phase0d_dark_refine(const fxp32 *in_q20,
  ...
  *sigma_sq_out = dyn_scaled >> 10;       /* unscale back to Q11.20 (Bug 11 reverted) */
}
...
static void phase2_dark_ref(const fxp32 *in_q20, const fxp32 *in_gat_q20,
   ...
   * α < 4.88e-4, causing fxp_recip to saturate.  σ²/α itself fits Q11.20. */
