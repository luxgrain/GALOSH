...
static void fxp_gat_build_inverse_table(const fxp_gat_params *gat_p) {
  /* Allow LUT build for any positive α.  For very small α (= λ_max overflows
   * Q11.20), the LUT build saturates high-x entries; the lookup detects
   * saturation and falls back to algebraic for D values above the valid
   * range.  Pre-Bug-11, this caused catastrophic output because σ²/α was
   * wrong; with Bug 11 fix (σ² precision preserved via scaled domain), the
   * partial LUT is correct for low-D and algebraic handles high-D. */
  if(gat_p->alpha < 1) {       /* skip only on absurd α (= 0 or near-zero) */
  ...
  /* Track last valid entry: for very small α, λ = x/α may saturate Q11.20
   * at high x.  We stop building when λ saturates and set d_max to last
   * valid d[i].  Lookup falls back to algebraic for D > d_max. */
  int valid_max_i = FXP_GAT_INV_TABLE_SIZE - 1;
  for(int i = 0; i < FXP_GAT_INV_TABLE_SIZE; i++) {
    ...
    if(lambda >= FXP_MAX_INT - 100) {
      /* λ saturated — stop building.  Lookup will use algebraic for D values
       * above d[valid_max_i]. */
      valid_max_i = (i > 0) ? (i - 1) : 0;
      break;
    }
  ...
  g_gat_inv_table.d_max = g_gat_inv_table.d[valid_max_i];
...
static void phase0c_wls_solve(const fxp32 *bin_mean_arr,
  ...
  /* Initial WLS defaults in SCALED domain (= 1024× of real default).
   * Bug 13 fix (2026-05-14): old code set initials in unscaled domain
   * (1e-4 raw, 1 raw); WLS internal works on 1024×-scaled variance, and
   * the >>10 unscale at end mapped unconverged initials to 1 raw raw,
   * i.e., effective α=9.5e-7, σ²=9.5e-7 — way smaller than FP32's default
   * 1e-4/1e-6 used for "Phase 0 fail" fallback.  Caused mid-ISO RawNIND
   * regression on scenes where WLS fails to find positive slope.
   * Defaults: α_real=1e-4 → scaled 1024×1e-4=0.1024 → raw=107374;
   *           σ²_real=1e-6 → scaled 1024×1e-6=1.024e-3 → raw=1074. */
  fxp32 alpha_est = 107374;       /* = 1024 × 1e-4 in Q11.20 raw */
  fxp32 sigma_sq_est = 1074;      /* = 1024 × 1e-6 in Q11.20 raw */
  ...
  /* Unscale α by /1024 (= >>10).  α has plenty of precision (4-5 LSBs minimum
   * even after unscale), so truncation is safe.
   *
   * Do NOT unscale σ² — keep in 1024×-scaled domain.  Q11.20 unscaled σ² for
   * super-clean signals (σ² < 1e-6) underflows to 0; the scaled form retains
   * 1+ LSBs.  fxp_gat_precompute_scaled and phase2 handle the 1024× factor
   * internally via fxp_div_q20 with appropriately-scaled denominators.
   * See Bug 11 commentary in fxp_gat_precompute_scaled (galosh_cpu_int.h). */
  alpha_est    = alpha_est    >> 10;     /* /1024 */
  /* sigma_sq_est: KEEP scaled (= 1024×σ²_real) */

  if(alpha_est < 1) alpha_est = 1;
  *alpha_out    = alpha_est;
  *sigma_sq_out = sigma_sq_est;     /* scaled output, downstream knows */
}
...
static void phase0d_dark_refine(const fxp32 *in_q20,
  ...
  /* Output σ²_scaled (= 1024 × σ²_real) — see Bug 11 fix at phase0c output. */
  *sigma_sq_out = dyn_scaled;
}
...
static void fxp_estimate_noise(const fxp32 *in_q20,
                               int width, int height,
                               fxp32 *alpha_out, fxp32 *sigma_sq_out) {
  /* Defaults in case Phase 0 fails (= "too few valid bins"):
   * α = 1e-4 (unscaled — α output is always unscaled Q11.20).
   * σ²_scaled = 1024 × 1e-6 (= scaled — σ² output convention is scaled). */
  *alpha_out    = fxp_from_float(1e-4f);
  *sigma_sq_out = 1074;     /* = 1024 × 1e-6 in Q11.20 raw */
...
static void phase2_dark_ref(const fxp32 *in_q20, const fxp32 *in_gat_q20,
                            int width, int height,
                            fxp32 alpha_q20, fxp32 sigma_sq_scaled_q20,
                            fxp32 ch_dark_ref[4]) {
  /* s_init = σ²/α.  σ² input is in SCALED form (= 1024 × σ²_real) per Bug 11
   * convention.  σ²/α = σ²_scaled / (1024 × α_real_q20).  α × 1024 fits int32
   * for α < 2 (= always true for our α values). */
  fxp32 alpha_x_1024 = (alpha_q20 < 1 ? 1 : alpha_q20) * 1024;
  fxp32 s_init = fxp_div_q20(sigma_sq_scaled_q20, alpha_x_1024);
...
int main(int argc, char **argv) {
  ...
  /* Bug 11 fix: σ² output is in SCALED form (= 1024 × σ²_real_q20).  This
   * preserves precision for super-clean signals (σ²<1e-6) that would
   * otherwise underflow Q11.20 unscaled storage.  Display divides by 1024
   * to show σ²_real; downstream uses fxp_gat_precompute_scaled. */
  fxp32 alpha_q20, sigma_sq_scaled_q20;
  if(alpha_in > 0.0f && sigma_sq_in >= 0.0f) {
    alpha_q20    = fxp_from_float(alpha_in);
    /* CLI provides σ²_real; scale up to match Phase 0 internal convention. */
    sigma_sq_scaled_q20 = fxp_from_float(sigma_sq_in) << 10;
    fprintf(stderr, "  Phase 0 (CLI override): alpha=%.6f sigma_sq=%.10f\n",
            fxp_to_float(alpha_q20), fxp_to_float(sigma_sq_scaled_q20 >> 10));
  } else {
    clock_t tp0 = clock();
    fxp_estimate_noise(in_q20, width, height, &alpha_q20, &sigma_sq_scaled_q20);
    double dt0 = (double)(clock() - tp0) / CLOCKS_PER_SEC;
    /* For display, divide scaled by 1024 to recover σ²_real (may lose precision
     * for very small σ², but it's display only — downstream uses scaled). */
    fprintf(stderr, "  Phase 0 (INT estimate, %.3f s): alpha=%.6f sigma_sq=%.10f "
            "(Q11.20: %d, scaled %d = real ~%.3e)\n",
            dt0, fxp_to_float(alpha_q20),
            fxp_to_float(sigma_sq_scaled_q20) / 1024.0f,
            alpha_q20, sigma_sq_scaled_q20,
            fxp_to_float(sigma_sq_scaled_q20) / 1024.0);
  }

  /* ========== Phase 1: GAT forward + per-CFA σ + unified_sigma normalize ========== */
  fxp_gat_params gat_p;
  fxp_gat_precompute_scaled(&gat_p, alpha_q20, sigma_sq_scaled_q20);
  ...
  phase2_dark_ref(in_q20, out_q20, width, height,
                  alpha_q20, sigma_sq_scaled_q20, ch_dark_ref);
