                                   ...
                                   const uint32_t filters)
{
  ...
  /* Phase 0: Noise parameter estimation (shared GAT) */
  const bm3d_noise_params_t np = bm3d_estimate_noise(in, width, height);
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "Raw Denoiser Standalone v4 (darktable rawdenoise.c)\n");
  ...
  {
    gat_bm3d_denoise_cfa(in, out, &roi, strength, 0 /* RGGB filters placeholder */);
  }
